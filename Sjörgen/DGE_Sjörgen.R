## 20251016
## DGE: Sjörgen vs. healthy (There is no teratment in this dataset!)
...
directory <- "/Users/sara/Desktop/Autocore/Sjörgen/counts/"
setwd(directory)
outputPrefix <-"Sjörgen_"
meta_Sjörgen<- read.csv("../Sjorgen_metadata.csv")

sampleFiles <- list.files(pattern = "_counts.txt$")
sampleNames <- sub("_counts.txt$", "", sampleFiles)
sampleCondition <- c(rep("Healthy",7),rep("Sjörgen",43))
...
#### Quality Control ####
pdf("../plots/Sjörgen_Dispersion_Plot.pdf", width = 11.69, height = 8.27)  # inches
par(mar = c(10, 5, 2, 2))  # bottom, left, top, right
plotDispEsts(dds)
dev.off()

pdf("../plots/Sjörgen_Boxplot_normalized_counts.pdf", width = 11.69, height = 8.27) 
par(mar = c(10, 5, 2, 2)) 
...
# Plot heatmap (SDM)
library(pheatmap)
pdf("../plots/Sjörgen_Heatmap1.pdf", width = 11.69, height = 8.27) 
pheatmap(vst_cor)
dev.off()


pdf("../plots/Sjörgen_PCA.pdf", width = 11.69, height = 8.27) 
...
resultsNames(dds)
resLFC <- lfcShrink(dds, coef="condition_Sjörgen_vs_Healthy", type="ashr")
resLFC
...
write.csv(resdata, file = paste0("../tables/Sjörgen-results-with-normalized.csv"))
...
pdf("../plots/Sjörgen_Volcano_Plot.pdf", width = 11.69, height = 8.27) 
EnhancedVolcano(res,
                lab = res$hgnc_symbol,
                x = 'log2FoldChange',
                y = 'pvalue',
...
x
pdf("../plots/Sjörgen_MA_Plot2.pdf", width = 11.69, height = 8.27) 
x+geom_point(aes(colour = padj))+
  scale_color_gradient(low="red", high="green")+
  geom_text_repel(aes(label=ifelse(log2FoldChange> 3,as.character(gene),'')),hjust=0.5,vjust=0.5,size=3)+
...
pdf("../plots/Sjörgen_Heatmap2.pdf", width = 11.69, height = 8.27) 
...
pdf("../plots/Sjörgen_Heatmap_Genes.pdf", width = 11.69, height = 8.27) 
heatmap.2(assay(vst)[select,], col=my_palette,
          scale="row", key=F, keysize=1, symkey=T,
          density.info="none", trace="none",
          cexCol=0.6, labRow=F,
          main="Heatmap of the top 500 DEGs in Sjörgen vs. Healthy")
dev.off()
