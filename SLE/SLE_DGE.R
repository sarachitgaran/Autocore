...
directory <- "/Users/sara/Desktop/Autocore/SLE/counts/SLET_vs_SLE/"
setwd(directory)
outputPrefix <-"SLE_"

### --- Re-run the analysis without the outlier (SRR33775210)
sampleFiles <- list.files(pattern = "_counts.txt$")
sampleNames <- sub("_counts.txt$", "", sampleFiles)
sampleCondition <- factor(c(rep("SLET",6),rep("SLE",5)))
...
pdf("../../plots/boxplot_normalized_counts.pdf", width = 11.69, height = 8.27)  # inches
...
pdf("../../plots/PCA_with_Outlier.pdf", width = 11.69, height = 8.27) 
...
resultsNames(dds)
resLFC <- lfcShrink(dds, coef="condition_SLET_vs_SLE", type="ashr")
resLFC
...
pdf("../../plots/SLE_Volcano_Plot.pdf", width = 11.69, height = 8.27) 
EnhancedVolcano(res,
                lab = res$hgnc_symbol,
                x = 'log2FoldChange',
                y = 'pvalue',
...
x
pdf("../../plots/SLE_MA_Plot2.pdf", width = 11.69, height = 8.27) 
...
pdf("../../plots/SLE_Heatmap2.pdf", width = 11.69, height = 8.27) 
...
pdf("../../plots/SLE_Heatmap_Genes.pdf", width = 11.69, height = 8.27) 
